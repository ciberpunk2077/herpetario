    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.search) {
            ...
            html.push(
                ...
                sprintf('<input class="forms-control' + (this.options.iconSize === undefined ? '' : ' input-' + this.options.iconSize) + '" type="text" placeholder="%s">',
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            if (!column.filterControl) {
                ...
                switch (column.filterControl.toLowerCase()) {
                    case 'input' :
                        html.push(sprintf('<input type="text" class="forms-control" style="width: 100%; visibility: %s">', isVisible));
                        break;
                    case 'select':
                        html.push(sprintf('<select class="%s forms-control" style="width: 100%; visibility: %s"></select>',
...
(function($) {
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        $sortModal.find('tbody')
            .append($('<tr>')
                .append($('<td>').text(text))
                .append($('<td>').append($('<select class="forms-control multi-sort-name">')))
                .append($('<td>').append($('<select class="forms-control multi-sort-order">')))
            );
...
!function($) {
    ...
    var createFormAvd = function(pColumns, searchText, that) {
        var htmlForm = [];
        htmlForm.push(sprintf('<forms class="forms-horizontal" id="%s" action="%s" >', that.options.idForm, that.options.actionForm));
        for (var i in pColumns) {
            var vObjCol = pColumns[i];
            if (!vObjCol.checkbox && vObjCol.visible && vObjCol.searchable) {
                htmlForm.push('<div class="forms-group">');
                htmlForm.push(sprintf('<label class="col-sm-4 control-label">%s</label>', vObjCol.title));
                htmlForm.push('<div class="col-sm-6">');
                htmlForm.push(sprintf('<input type="text" class="forms-control input-md" name="%s" placeholder="%s" id="%s">', vObjCol.field, vObjCol.title, vObjCol.field));
                htmlForm.push('</div>');
                htmlForm.push('</div>');
            }
        }
